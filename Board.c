...
int countMines(struct Board *b, int row, int col)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if ((checkRow >= 0 && checkRow < b->size) && (checkCol >= 0 && checkCol < b->size))
        {
...
// Recursively Digs For Empty Cells
bool revealCell(struct Board *b, int row, int col)
{
    ...
    { // 0 = empty cell
        ...
        if (count == 0)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                int nextRow = row + xVal[i];
                int nextCol = col + yVal[i];

                revealCell(b, nextRow, nextCol);
            }
        }
    }
    return false;
